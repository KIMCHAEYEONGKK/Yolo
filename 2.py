...
from networks.depth_encoder_only_ghostinCDC import LiteMono
...
def run_video_inference(weights_folder, video_path):
    ...
    while cap.isOpened():
        ...
        # depth_threshold = 180
        # max_brightness = max_brightness_in_region
        # # 기본값은 직진
        # avoidance_direction = "Center"

        # # 중앙이 밝기 기준을 초과한 경우에만 회피 판단
        # if max_brightness_in_region["Center"] >= depth_threshold:
        #     left_brightness = max(max_brightness["Left"], 0)
        #     right_brightness = max(max_brightness["Right"], 0)
            ...
            if 0 <= cx < disp_np.shape[1] and 0 <= cy < disp_np.shape[0]:
                ...
                if disparity > 0:
                    ...
                    if cx < region_width:
                        ...
                        region = "Right"
                    max_brightness_in_region[region] = max(max_brightness_in_region[region], brightness)
                else:
                    label = "N/A"
